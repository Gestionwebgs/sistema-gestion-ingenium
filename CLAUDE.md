...
3. **Gastos generales de la empresa sin proyecto asociado — confirmado, falta pantalla de detalle**: las hojas mensuales "Compras y gastos [MES]" registran gastos marcados "PAGO INGENIUM" que no pertenecen a ningún proyecto específico. Se probó en la demo de la segunda máquina (clasificando una captura con "Sin proyecto (general)"): el gasto **sí se guarda correctamente** (`project_id` nulo, se refleja bien en la tarjeta "Gastos generales" de `/panel`). Lo que falta es una pantalla que liste esos gastos generales uno por uno — hoy `/panel` solo muestra la suma total, y el "Registro de Gastos" con detalle línea por línea solo existe dentro de cada proyecto (`/proyectos/[id]`). Falta construir el equivalente para gastos/abonos sin proyecto.
