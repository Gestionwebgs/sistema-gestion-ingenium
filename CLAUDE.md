...
## Despliegue en AWS con Terraform (2026-08-29)

El usuario ya tiene la cuenta de AWS y un usuario IAM administrador (sin usar
root). Pidió desplegar en una sola instancia EC2 ("instalamos todo desde
ahí"), sin dominio todavía, con el máximo de seguridad/respaldo posible y
cuidando que no se repita el problema ya conocido de que la app se cuelga
con poca RAM (pasó antes con 2GB). Guía completa, con cada comando, en
`docs/DESPLIEGUE_AWS.md` — acá solo el resumen de las decisiones:

- **Instancia**: t3.medium (4GB RAM), elegida explícitamente por el usuario
  en vez de la capa gratuita (t2/t3.micro, 1GB) — con Cotizaciones y
  Facturación ya sumadas al sistema, 1GB se quedaría corto seguro. Esto
  hace que el despliegue ya NO sea gratis (~USD 33-35/mes), decisión
  consciente del usuario priorizando estabilidad sobre costo cero.
- **Todo en Terraform** (`infra/`), no clicks manuales en la consola — así
  queda documentado y se puede recrear. Recursos: EC2 + IP elástica,
  security group (SSH restringido a la IP del usuario, 80/443 públicos),
  bucket S3 (privado, versionado, cifrado — reemplaza a MinIO en
  producción), rol IAM de la instancia con permiso SOLO a ese bucket (nunca
  hay una access key guardada en el servidor), snapshots automáticos
  diarios del disco (AWS DLM, 7 días de retención), alarmas opcionales de
  CloudWatch si memoria/disco se disparan (requiere completar
  `alert_email` en `terraform.tfvars`).
- **Dentro del servidor**: Postgres en Docker (igual que en desarrollo
  local), la app Next.js corriendo nativa vía **systemd**
  (`deploy/ingenium-app.service`, `Restart=always`) en vez de PM2 o una
  terminal abierta — esto es lo que reemplaza definitivamente al viejo
  pendiente de "resiliencia del proceso" que se había marcado como
  innecesario pensando que AWS no lo iba a necesitar; en realidad SÍ hace
  falta algo que reinicie la app sola, systemd lo resuelve sin agregar
  ninguna pieza nueva. Caddy (Docker) como reverse proxy: sirve por HTTP
  simple mientras no haya dominio, y da HTTPS automático (Let's Encrypt)
  solo con cambiar una línea del `Caddyfile` cuando el dominio esté listo.
- **`src/lib/s3.ts`**: se hizo opcional el bloque `credentials` del
  `S3Client` (antes exigía `S3_ACCESS_KEY_ID`/`SECRET` siempre). En el
  servidor de AWS esas variables se dejan vacías a propósito, y el SDK
  toma las credenciales solo del rol IAM de la instancia — en local
  (MinIO) sigue funcionando igual que antes, sin cambios de comportamiento.
- **No se creó** una instancia RDS ni App Runner (estaba en el plan
  original del backlog) — se simplificó todo a una sola EC2 por pedido
  explícito del usuario. Queda anotado como posible paso futuro si el uso
  crece mucho: separar Postgres a RDS gestionado.
- **Pendiente real**: el usuario todavía no corrió `terraform apply` ni el
...
## Nota: recuperación de una pérdida accidental de documentación (2026-09-04)

El commit de despliegue en AWS (`a63eed7`, hecho en la otra computadora) llegó con `CLAUDE.md` basado en una copia vieja del archivo — probablemente un editor con el archivo abierto desde antes de hacer `git pull`, que sobreescribió el archivo entero al guardar. Resultado: además de agregar la sección de AWS (que sí quedó bien y se conservó tal cual), el commit **borró** las secciones "Revisión cruzada", "Préstamos de terceros: tasa de interés", "Cotizaciones" y "Facturación" (las 4 de arriba, ahora restauradas), y **revirtió** la nota del bug de `/capturas` a su versión "no corregir todavía" aunque el código arreglado seguía intacto. Ningún archivo de código ni migración se vio afectado — se verificó con `git show --stat` que el commit solo tocó archivos de `infra/`, `deploy/`, `docker-compose.prod.yml`, `Caddyfile`, `.env.production.example`, `src/lib/s3.ts` y `CLAUDE.md`. Se restauró el contenido perdido a mano comparando contra el commit anterior (`b841927`). **Lección para la próxima**: antes de editar `CLAUDE.md` a mano en cualquier computadora, asegurarse de tener el archivo recién traído con `git pull`, no una pestaña de editor vieja.
