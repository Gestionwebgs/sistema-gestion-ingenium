# Contexto del Proyecto — Sistema de Gestión de Proyectos Ingenium Service SAC
...
- No asumir ningún requerimiento que no esté confirmado aquí; preguntar antes de asumir.
- Primer entregable: generar CLAUDE.md con este contexto + una propuesta de modelo de datos, y validarla con el usuario ANTES de escribir código.
- País: Perú. Tener en cuenta normativa/terminología local: RUC, IGV (18%), SCTR, moneda Soles (S/.).
...
- Contacto: +51 974 823 889 / +51 999 997 757 · proyectos@ingeniumservicesac.com · www.ingeniumservicesac.com

## Líneas de negocio (5, cada una con sus propios proyectos)
...
1. **Datos generales del proyecto**: nombre, ubicación, cliente, usuario/contacto del cliente, empresa cliente, RUC, fecha inicio/fin, responsable por Ingenium, orden de compra, línea de negocio a la que pertenece.
...
3. **Problema clave a resolver — control de compras/facturas**: el dueño de la empresa compra insumos y servicios a veces con su cuenta personal y a veces con la cuenta de la empresa. Hoy eso no se registra de forma ordenada. El sistema debe permitir, por cada gasto:
...
## Ejemplo real de estructura de datos (Excel actual del cliente, proyecto "CIMENTACION")
...
Este archivo real está disponible para usar como caso de prueba de importación/migración de datos.
...
## Identidad visual (extraída del brochure oficial 2026)
...
- Repositorio en GitHub: crear el repo del proyecto (usando `gh repo create` o similar) como parte del setup inicial.

## Stack tecnológico (confirmado)
- Next.js + TypeScript + Tailwind CSS (frontend, con la paleta de marca de arriba)
...
## Primer paso a ejecutar (en Claude Code)
1. Crear el repositorio en GitHub.
2. Generar CLAUDE.md del proyecto con todo este contexto (empresa, líneas de negocio, problema de control de gastos, stack, plan de despliegue).
3. Proponer y validar con el usuario el modelo de datos (tablas y relaciones) antes de escribir cualquier código. NO avanzar a escribir código sin esa validación explícita.
4. A partir de ahí, avanzar paso a paso, esperando confirmación del usuario después de cada paso antes de continuar con el siguiente. No asumir requerimientos no confirmados.
